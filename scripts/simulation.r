...
fit_sdms <- function(community, sites, method = "glm"){
  sites_df <- as.data.frame(sites)
  sites2 <- cbind(sites, sites^2)
  sites2_df <- as.data.frame(sites2)
  apply(community, 2, function(x){
    if(sum(x) == 0) return(rep(NA, nrow(community)))
    if(method == "maxent"){
      fit <- dismo::maxent(sites_df, x)
      return(predict(fit, sites_df, type = "response"))
    }
    if(method == "glm"){
      fit <- glm(x ~ sites2, family = binomial)
...
simulate <- function(n_sites = 10000, 
  ...
  fits <- fit_sdms(community$occ, sites, method = sdm_method)
